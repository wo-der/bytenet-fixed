...
declare type struct<T extends { [index: string]: ByteNetType<any> }> = ByteNetType<{
	[valueName in keyof T]: T[valueName]["value"];
}>;
declare type optional<T extends ByteNetType<any>> = ByteNetType<T["value"] | undefined>;
declare type map<K extends ByteNetType<any>, V extends ByteNetType<any>> = ByteNetType<Map<K["value"], V["value"]>>;
...
declare namespace ByteNet {
	...
	export function struct<T extends { [index: string]: ByteNetType<any> }>(struct: T): struct<T>;
	export function optional<T extends ByteNetType<any>>(type: T): optional<T>;
	export function map<K extends ByteNetType<any>, V extends ByteNetType<any>>(key: K, value: V): map<K, V>;
}
